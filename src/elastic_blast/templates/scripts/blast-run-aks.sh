...
# Get database length for metadata (only if not already cached by another pod)
...
# On failure, upload error details (login to azcopy only here)
if [[ $BLAST_EXIT_CODE -ne 0 ]]; then
    azcopy login --identity 2>/dev/null || true
    if ! azcopy ls "$ELB_RESULTS/metadata/FAILURE.txt" &>/dev/null; then
        azcopy cp "$ERROR_FILE" "$ELB_RESULTS/metadata/FAILURE.txt" 2>/dev/null || true
    fi
