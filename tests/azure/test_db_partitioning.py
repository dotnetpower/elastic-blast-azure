...
from elastic_blast.constants import AKS_PROVISIONING_STATE, ElbCommand
...
def _make_cfg(
    ...
    skip_warmed_ssd_init: bool = False,
) -> ElasticBlastConfig:
    ...
    with (
        patch("elastic_blast.elb_config.get_latest_dir", return_value="latest"),
...
class TestSubmitPartitioned:
    ...
    @patch(
        "elastic_blast.azure.get_blastdb_info", return_value=("testdb", "", "testdb")
    )
    ...
    def test_submit_uses_standard_for_no_partitions(self, mock_usage, mock_dbinfo):
        ...
        with (
            patch.object(elb, "_submit_partitioned") as mock_partitioned,
            patch(
                "elastic_blast.kubernetes.submit_job_submission_job"
            ) as mock_submit_job,
        ):
            query_batches = ["batch_000.fa", "batch_001.fa"]
            elb.submit(query_batches, 1000, False)
    ...
    def test_partitioned_checks_job_limit(self, mock_usage, mock_dbinfo):
        ...
        with patch(
            "elastic_blast.kubernetes.get_maximum_number_of_allowed_k8s_jobs",
            return_value=100,
        ):
    ...
    def test_partitioned_submit_always_deploys_finalizer(self, mock_usage):
        ...
        with (
            patch.object(elb, "_generate_partitioned_jobs") as mock_generate,
...
class TestInitializeClusterPartitioned:
    ...
    def test_creates_cluster_and_inits_partitioned_storage(
        ...
        with (
            patch.object(elb, "_get_k8s_ctx", return_value="test-ctx"),
    ...
    @patch("elastic_blast.azure.get_usage_reporting", return_value=False)
    @patch(
        "elastic_blast.azure.check_cluster",
    ...
    def test_reuse_skips_cluster_creation(
        ...
        cfg = _make_cfg(
            db_partitions=4, db_partition_prefix=PARTITION_PREFIX, reuse=True
        )
        elb = ElasticBlastAzure(cfg)
        elb.cloud_job_submission = False
...
"""
    ...
    def test_finalizer_template_includes_blast_options(self, mock_usage):
        ...
        cfg = _make_cfg(
            db_partitions=3, db_partition_prefix=PARTITION_PREFIX, dry_run=False
        )
    ...
    def test_merge_script_respects_max_target_seqs_and_writes_report(self):
        ...
        script = (
            ...
            / "src/elastic_blast/templates/scripts/merge-sharded-results.sh"
        )
        rows = [
    ...
    def test_merge_script_writes_valid_xml_for_outfmt_5(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            for shard, hits in {
                "shard_00": [
                    ("subject_slow", "1e-10", 80.0),
                    ("subject_best", "1e-30", 70.0),
                ],
    ...
    def test_merge_script_xml_tie_prefers_more_hsps(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            two_hsp = self._xml_result(
                "Query_1", [("subject_two_hsp", "1e-20", 80.0)]
            ).replace(
                "</Hit_hsps>",
                """<Hsp>
                <Hsp_num>2</Hsp_num>
    ...
    def test_finalizer_fails_incomplete_shard_downloads(self):
        ...
        script = (
            Path(__file__).parents[2]
            / "src/elastic_blast/templates/scripts/elb-finalizer-aks.sh"
        )
...
class TestInitializeStoragePartitioned:
    ...
    def test_creates_pvc_and_init_job(self):
        ...
        assert len(apply_calls) >= 2, (
            f"Expected at least 2 kubectl apply calls, got {len(apply_calls)}"
        )
    ...
    def test_uses_template_with_configmap_scripts(self):
        ...
        assert "elb-scripts" in yaml_content, "Expected ConfigMap name in volumes"
        # Env vars pass partition info to the script
        assert "ELB_NUM_PARTITIONS" in yaml_content
        assert '"3"' in yaml_content, "Expected num_partitions=3 in env var"
        assert PARTITION_PREFIX in yaml_content, "Expected partition prefix in env var"
...
class TestInitializeLocalSsdSharded:
    ...
    def test_skips_init_jobs_when_dashboard_warmup_ready(self):
        ...
        cfg = _make_cfg(
            db_partitions=3,
            db_partition_prefix=PARTITION_PREFIX,
            reuse=True,
            dry_run=False,
            use_local_ssd=True,
        ...
        assert any(
            "get jobs -l app=elb-db-warmup,db=RNAvirome.S2.RDRP" in c
...
class TestMergePartitionedResults:
    ...
    def test_merge_dry_run_logs_only(self):
        ...
        cfg = _make_cfg(
            db_partitions=2, db_partition_prefix=PARTITION_PREFIX, dry_run=True
        )
        elb = ElasticBlastAzure.__new__(ElasticBlastAzure)
        elb.cfg = cfg
        elb.dry_run = True
